...
class IntegrationTestRunner:
    ...
    def setup_test_environment(self) -> bool:
        ...
        if not self.scale_service("data-ingestion", 0):
            return False
            
        if not self.restart_service("ma-signal-detector"):
            return False
            
        if not self.restart_service("volume-spike-detector"):
            return False
    ...
    def cleanup_test_environment(self) -> bool:
        ...
        if not self.scale_service("data-ingestion", 1):
            return False
    ...
    def verify_signals(self, signals: List[Dict], expected_type: str, expected_count: int) -> bool:
        matching_signals = [s for s in signals if s.get('signal_type') == expected_type]
        
        if len(matching_signals) == expected_count:
            logger.info(f"✓ {expected_type} test passed: {expected_count} signals received")
            return True
        else:
            logger.error(f"✗ {expected_type} test failed: expected {expected_count}, got {len(matching_signals)}")
            return False
            
    def run_scenario_test(self, scenario_name: str, events: List[Dict], expected_signal_type: str, expected_count: int) -> bool:
    ...
    def run_all_tests(self) -> bool:
        ...
        try:
            golden_cross_events = self.data_generator.generate_golden_cross_scenario()
            result = self.run_scenario_test("Golden Cross", golden_cross_events, "moving_average_crossover", 1)
            test_results.append(("Golden Cross", result))
            
            time.sleep(5)
            
            death_cross_events = self.data_generator.generate_death_cross_scenario()
            result = self.run_scenario_test("Death Cross", death_cross_events, "moving_average_crossover", 1)
            test_results.append(("Death Cross", result))
            
            time.sleep(5)
            
            volume_spike_events = self.data_generator.generate_volume_spike_scenario()
            result = self.run_scenario_test("Volume Spike", volume_spike_events, "volume_spike", 1)
